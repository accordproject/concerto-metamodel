...
function findNamespace(priorModels, namespace) {
    ...
    // Parse range-based versioning (supports x-ranges like 1.x, 1.1.x)
    ...
    // Treat requested version as a range (supports x-ranges like 1.x, 1.1.x)
...
function createNameTable(priorModels, metaModel) {
    ...
    (metaModel.imports || []).forEach((imp) => {
        ...
        if (imp.$class === `${MetaModelNamespace}.ImportType`) {
            ...
            table[imp.name] = {namespace: modelFile.namespace, name: imp.name};
            ...
            imp.types.forEach((type) => {
                ...
                table[localName] = localName !== type ? {namespace: modelFile.namespace, name: localName, resolvedName: type} : {namespace: modelFile.namespace, name: type};
            });
        } else {
            (modelFile.declarations || []).forEach((decl) => {
                table[decl.name] = {namespace: modelFile.namespace, name: decl.name};
            });
        }
    });
