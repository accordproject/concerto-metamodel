...
function createNameTable(priorModels, metaModel) {
    ...
    const table = {
        'Concept': {namespace: concertoNs, name: 'Concept', resolvedName: 'Concept'},
        'Asset': {namespace: concertoNs, name: 'Asset', resolvedName: 'Asset'},
        'Participant': {namespace: concertoNs, name: 'Participant', resolvedName: 'Participant'},
        'Transaction': {namespace: concertoNs, name: 'Transaction', resolvedName: 'Transaction'},
        'Event': {namespace: concertoNs, name: 'Event', resolvedName: 'Event'},
    };
    ...
    (metaModel.imports || []).forEach((imp) => {
        ...
        if (imp.$class === `${MetaModelNamespace}.ImportType`) {
            ...
            table[imp.name] = {namespace, name: imp.name, resolvedName: imp.name};
            ...
            imp.types.forEach((type) => {
                ...
                table[localName] = {namespace, name: localName, resolvedName: type};
            });
        } else {
            (modelFile.declarations || []).forEach((decl) => {
                // What is this used for?
                table[decl.name] = {namespace, name: decl.name, resolvedName: decl.name};
            });
        }
    });
    ...
    (metaModel.declarations || []).forEach((decl) => {
        table[decl.name] = {namespace: metaModel.namespace, name: decl.name, resolvedName: decl.name};
    });
...
function resolveTypeNames(metaModel, table) {
    ...
    case `${MetaModelNamespace}.IntegerScalar`: {
        ...
        metaModel.resolvedName = table[metaModel.name].resolvedName;
    }
        break;
    }
    return metaModel;
